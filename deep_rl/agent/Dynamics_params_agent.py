...
class Dynamics_Param_Agent(BaseAgent):
    def __init__(self, config):
    ...
    def train_mdp(self, alpha=None, train_steps=None):
        ...
        for trian_iter in range(train_steps):
            ...
            self.mdp.generate_theta(sample_z)

            reward_theta = self.mdp.reward_theta
            mdp_theta = self.mdp.mdp_theta
            ...
            """ Point of consideration for jerry # 1 splitting the gradient """
            reward_loss = (rewards.detach() - pred_rewards).pow(2).mul(0.5)
            reward_grad = autograd.grad(reward_loss.sum(), inputs=reward_theta)[0]
            reward_grad_i, reward_grad_j = torch.split(reward_grad, config.particles//2, dim=0)
            ...
            # theta_mdp_i, theta_mdp_j = torch.split(mdp_theta, config.particles//2, dim=0)
            theta_reward_i, theta_reward_j = torch.split(reward_theta, config.particles//2, dim=0) # [p/2, theta]
            ...
            ## reward grad w.r.t theta j
            # reward_grad_j = reward_grad_j.unsqueeze(2)  # [batch, particles//2. 1, 1]
            ...
            # kernel performed w.r.t theta 
            kappa_reward, grad_kappa_reward = batch_rbf_xy(theta_reward_j, theta_reward_i) # [1, p/2, p/12], [1, p/2, p/2, theta]

            kernel_reward_grad = torch.matmul(kappa_reward.detach(), reward_grad_j)  # [1, p/2, theta]
            kernel_reward_grad = kernel_reward_grad.squeeze(0) # [p/2, theta]
            svgd_reward = (kernel_reward_grad + alpha * grad_kappa_reward) # [1, p/2, p/2, theta]

            """ Question 2 for jerry, is the mean over either p/2 dimension ok? """

            svgd_reward = svgd_reward.mean(1).squeeze(0) # [p/2, theta]

            self.mdp_optimizer.zero_grad()
            #autograd.backward(pred_rewards_i, grad_tensors=svgd_reward.detach(), retain_graph=True)
            autograd.backward(theta_reward_i, grad_tensors=svgd_reward.detach(), retain_graph=True)
            ...
            """ same issue # 1, splitting grads """
            mdp_grad = autograd.grad(mdp_loss.sum(), inputs=mdp_theta)[0]
            mdp_grad_i, mdp_grad_j = torch.split(mdp_grad, config.particles//2, dim=0)

            # mdp_grad_j = mdp_grad_j.unsqueeze(2) # [batch, particles//2, 1, d_state]

            theta_mdp_i, theta_mdp_j = torch.split(mdp_theta, config.particles//2, dim=0) # [p/2, theta]

            # grad_kappa: [batch, particles/2, particles/2, d_state+1]
            #kappa_mdp, grad_kappa_mdp = batch_rbf_xy(pred_states_j, pred_states_i)
            kappa_mdp, grad_kappa_mdp = batch_rbf_xy(theta_mdp_j, theta_mdp_i)
            # kappa_mdp = kappa_mdp.unsqueeze(-1)  # [batch, particles/2, particles/2, 1]

            kernel_mdp_grad = torch.matmul(kappa_mdp.detach(), mdp_grad_j)
            kernel_mdp_grad = kernel_mdp_grad.squeeze(0)
            svgd_mdp = (kernel_mdp_grad + alpha * grad_kappa_mdp)
            """ same question # 2 """
            svgd_mdp = svgd_mdp.mean(1).squeeze(0)

            autograd.backward(theta_mdp_i, grad_tensors=svgd_mdp.detach())
