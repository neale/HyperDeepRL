...
class DQN_SGD_Agent(BaseAgent):
    ...
    def step(self):
        ...
        if self.total_steps > self.config.exploration_steps:
            ...
            moment1_loss_j = (q_next_frozen.mean(1) - q_frozen.mean(1)).pow(2).mul(.5).mean()
            moment2_loss_j = (q_next_frozen.var(1) - q_frozen.var(1)).pow(2).mul(.5).mean()
            action_loss_j = (q_next_frozen - q_a_frozen).pow(2).mul(0.5)
            sample_loss_j = (q_next_frozen - q_frozen).pow(2).mul(0.5) 
