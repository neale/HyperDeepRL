    ...
    def record_online_return(self, info, offset=0):
        ...
        if isinstance(info, dict):
            ...
            if ret is not None:
                ...
                self.logger.add_scalar('Posterior_var_actor', q_var, self.total_steps + offset)
                self.logger.add_scalar('episode', ep, self.total_steps + offset)
                self.logger.info('ep: %d| steps: %s| total_steps: %d| return_train: %.3f| ep_upright: %s| total_upright: %s| total_return: %.3f| posterior_var: %.4f' % (
                    ep,
                    steps,
